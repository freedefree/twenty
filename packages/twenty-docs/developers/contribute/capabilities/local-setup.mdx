...
```bash
docker compose -f packages/twenty-docker/docker-compose.remika-dev.yml up -d
```
...
Twenty's frontend / app entry will be running at [http://localhost:3100](http://localhost:3100) when using the Docker stack. If you start `npx nx start twenty-front` directly, the dev server defaults to [http://localhost:3099](http://localhost:3099).
You can log in using the default demo account: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- Twenty's server will be up and running at [http://localhost:3100](http://localhost:3100)
- The GraphQL API can be accessed at [http://localhost:3100/graphql](http://localhost:3100/graphql)
- The REST API can be reached at [http://localhost:3100/rest](http://localhost:3100/rest)
