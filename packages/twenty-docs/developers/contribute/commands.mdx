...
```bash
docker compose -f packages/twenty-docker/docker-compose.remika-dev.yml up -d   # Local Docker stack (http://localhost:3100)
npx nx start twenty-front          # Frontend dev server (http://localhost:3099)
```
